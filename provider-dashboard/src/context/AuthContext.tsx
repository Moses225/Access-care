...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const isMFAEnrolled = !!(user && multiFactor(user).enrolledFactors.length > 0);
  ...
  const completeMFAEnrollment = async (code: string) => {
    if (!user) throw new Error('Not authenticated');
    const cred = PhoneAuthProvider.credential(enrollVerificationIdRef.current, code);
    ...
    // Refresh user object to reflect new enrollment
  ...
  const completeMFAChallenge = async (code: string) => {
    ...
    const cred = PhoneAuthProvider.credential(challengeVerificationIdRef.current, code);
    const assertion = PhoneMultiFactorGenerator.assertion(cred);
  ...
  return (
    <AuthContext.Provider value={{
      user, providerProfile, loading, isMFAEnrolled, mfaResolver,
      login, logout,
      startMFAEnrollment, completeMFAEnrollment, cancelMFAEnrollment,
