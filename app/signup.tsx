...
const sanitizeName = (name: string): string =>
  name.trim().replace(/[^a-zA-Z\s'-]/g, '').substring(0, 50);
...
const validateName = (name: string, field: string): string | null => {
  ...
  if (!/^[a-zA-Z\s'-]+$/.test(name.trim())) return `${field} contains invalid characters.`;
  return null;
};
...
export default function SignupScreen() {
  ...
  const handleSignup = async () => {
    ...
    if (firstNameError) { Alert.alert('Validation Error', firstNameError); return; }

    const lastNameError = validateName(lastNameClean, 'Last name');
    if (lastNameError) { Alert.alert('Validation Error', lastNameError); return; }
    ...
    try {
      ...
      const errorMessage = errorMap[error.code] ?? 'Failed to create account. Please try again.';
...
const styles = StyleSheet.create({
  ...
  passwordHint: { fontSize: 12, marginTop: -8, marginBottom: 14, paddingHorizontal: 4, lineHeight: 18 },
